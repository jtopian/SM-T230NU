...
static int pm800_init_config(struct pm80x_chip *chip)
{
	...
	case CHIP_PM822:
		/* reading of slp_cnt before PM800_RTC_MISC6 is acceded*/
		//regmap_read(chip->regmap, PM800_RTC_MISC6, &data);
		//pr_info("%s reg 0xe8 is  %x\n", __func__,data);
		regmap_read(chip->regmap, PM800_RTC_MISC7, &data);
		slp_cnt_boot = data << 3;
		pr_info("%s slp_cnt at boot is %d\n", __func__,slp_cnt_boot);
