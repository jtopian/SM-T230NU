...
static bool __init sec_chg_gpio_init(void)
{
	return true;
}
			 ...
			 sec_battery_platform_data_t *pdata)
{
	...
	pdata->temp_high_threshold_event = 500;
	pdata->temp_high_recovery_event = 400;
	pdata->temp_low_threshold_event = 0;
	pdata->temp_low_recovery_event = 80;
	pdata->temp_high_threshold_normal = 500;
	pdata->temp_high_recovery_normal = 400;
	pdata->temp_low_threshold_normal = 0;
	pdata->temp_low_recovery_normal = 80;
	pdata->temp_high_threshold_lpm = 500;
	pdata->temp_high_recovery_lpm = 400;
	pdata->temp_low_threshold_lpm = 0;
	pdata->temp_low_recovery_lpm = 80;
#else
...
#if defined(CONFIG_MACH_DEGAS_BMW)
	pdata->full_condition_vcell = 4000;
#else
...
#if defined(CONFIG_MACH_DEGAS_BMW)
		pdata->recharge_condition_vcell = 4050;
#else
...
int sec_chg_irqgpio_dt_init(struct device_node *np,
			 struct device *dev,
			 sec_battery_platform_data_t *pdata)
{
	int ret = 0;
