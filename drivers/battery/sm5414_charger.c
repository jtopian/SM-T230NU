...
static int SM5414_get_charging_status_fg(struct i2c_client *client)
{
	int status = POWER_SUPPLY_STATUS_UNKNOWN;
	struct sec_charger_info *charger = i2c_get_clientdata(client);
	int nCHG;
	u8 chg_en, bat_status;
	...
	} else	if (bat_status == POWER_SUPPLY_STATUS_FULL) {
		status = POWER_SUPPLY_STATUS_FULL;
...
int sec_get_charging_health(struct i2c_client *client)
{
	...
	psy_do_property("battery", get, POWER_SUPPLY_PROP_HEALTH, val);
	health = val.intval;
	if (val.intval <= 0)
...
static u8 SM5414_get_float_voltage(
		struct i2c_client *client)
{
	...
	data &= 0xF0;
	data = (data >> 4);
	float_voltage = (data * 25) + 4100;

#if 0
	pr_debug("%s: battery float voltage: 0x%x, %d\n",
		__func__, data, float_voltage);
#endif
		...
		struct i2c_client *client, int float_voltage)
{
	...
	SM5414_i2c_read(client, SM5414_CHGCTRL3, &data);
	dev_info(&client->dev,
		"%s : SM5414_CHGCTRL3(0x%02x) : 0x%02x, float_voltage: %d\n",
		__func__, SM5414_CHGCTRL3, data, float_voltage);

	return data;
}

#if 0
static int SM5414_get_input_current_limit_data(
		struct i2c_client *client)
{
	u8 curr_reg = 0;
	int input_current;
	...
	curr_reg &= 0x3F;
	input_current = (curr_reg * 50) + 100;

	dev_info(&client->dev,
		"%s : SM5414_VBUSCTRL(0x%02x) : 0x%02x, limit: %d\n",
		__func__, SM5414_VBUSCTRL, curr_reg, input_current);
		...
		struct i2c_client *client, int topoff_current)
{
	...
	SM5414_i2c_read(client, SM5414_CHGCTRL4, &data);
	dev_info(&client->dev,
		"%s : SM5414_CHGCTRL4(0x%02x) : 0x%02x, Top-off limit: %d\n",
		__func__,SM5414_CHGCTRL4, data, topoff_current);

	return data;
}

#if 0
static int SM5414_get_fast_charging_current(
		struct i2c_client *client)
{
	u8 data = 0;
	int fast_charging_current = 0;

	SM5414_i2c_read(client, SM5414_CHGCTRL2, &data);
	data &= 0x3F;
	fast_charging_current = (data * 50) + 100;
	dev_info(&client->dev,
		"%s : SM5414_CHGCTRL2(0x%02x): 0x%02x, fast_charging_current : %d\n",
		__func__, SM5414_CHGCTRL2, data, fast_charging_current);
...
static u8 SM5414_set_toggle_charger(
...
static void SM5414_set_toggle_charger_gpio(
	struct i2c_client *client, int enable)
{
	int nCHG;
	struct sec_charger_info *charger = i2c_get_clientdata(client);
				...
				struct i2c_client *client)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		charger->charging_current = usb_charging_current;
		...
		if (charger->pdata->full_check_type_2nd == SEC_BATTERY_FULLCHARGED_CHGPSY) {
			...
			if (chg_mode.intval == SEC_BATTERY_CHARGING_2ND) {
				SM5414_set_toggle_charger_gpio(client, 0);
	...
	SM5414_set_toggle_charger_gpio(client, charger->is_charging);
}
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS_FG:
		if (charger->is_fullcharged)
			...
			val->intval = SM5414_get_charging_status_fg(client);
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = SM5414_get_float_voltage(client);
#if 0
		pr_debug("%s : SM5414_get_float_voltage : 0x%02x\n",
			__func__, val->intval);
#endif
		break;
#endif
			      ...
			      const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		SM5414_set_float_voltage_data(client, val->intval);
		break;
#endif
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		pr_info("%s: Always enable charger, enable value:%d\n",
			__func__, val->intval);
		if (val->intval) {
			SM5414_set_toggle_charger_gpio(client, 1);
		 } else {
			SM5414_set_toggle_charger_gpio(client, 0);
		 }
		break;
