...
static void irq_process_reset(struct mv_udc *udc)
{
	...
	if (readl(&udc->op_regs->portsc[0]) & PORTSCX_PORT_RESET) {
		u32 result = readl(&udc->pmu_regs->sd_rot_wake_clear);
		dev_info(&udc->dev->dev, "usb bus reset (vbus is %d)\n", result&0x1<<15 ? 1 : 0);
		udc->usb_state = USB_STATE_DEFAULT;
...
static int mv_udc_probe(struct platform_device *pdev)
{
	...
	udc->pmu_regs = (struct mv_cap_regs __iomem *)
		devm_ioremap(&pdev->dev, r->start+0x7A77C, resource_size(r));
	if (udc->pmu_regs == NULL) {
		dev_err(&pdev->dev, "failed to map pmu_regs I/O memory!!!!!!!!\n");
		return -EBUSY;
	}
