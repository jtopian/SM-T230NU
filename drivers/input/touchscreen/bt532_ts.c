...
#include "zinitix_touch_bt532_GFF_firmware.h"
#include "zinitix_touch_bt532_G1F_firmware.h"
u8* m_firmware_data;
#else
#include "zinitix_touch_bt532_firmware_degas3g.h"
#endif
...
//SDND
#define SEC_SDND_N_COUNT_00		10
#define SEC_SDND_U_COUNT_00		2
#define SEC_SDND_FREQUENCY_00		99 /* 200khz */

//PDND
#define SEC_PDND_N_COUNT_00		16
#define SEC_PDND_U_COUNT_00		14
#define SEC_PDND_FREQUENCY_00		66

/*---------------------------------------------------------------------*/
/* G1F */
/*---------------------------------------------------------------------*/
//SDND
#define SEC_SDND_N_COUNT_02			10
#define SEC_SDND_U_COUNT_02			1
#define SEC_SDND_FREQUENCY_02		101

//PDND
#define SEC_PDND_N_COUNT_02			16
#define SEC_PDND_U_COUNT_02			14
#define SEC_PDND_FREQUENCY_02		101
/*---------------------------------------------------------------------*/
...
static void bt532_firmware_check(void)
{
	...
	u8 ret;
	struct bt532_ts_platform_data *pdata = misc_info->pdata;
	int tsp_vendor_1 = pdata->tsp_vendor_1;
	int tsp_vendor_2 = pdata->tsp_vendor_2;

	misc_info->tsp_type = -1;

	ret = gpio_request(tsp_vendor_1, "TSP_ID_CHECK1");
	if (ret < 0) {
		pr_err("%s: Request GPIO failed, gpio %d (%d)\n", BT532_TS_DEVICE,
			tsp_vendor_1, ret);
		return;
	}
	ret = gpio_request(tsp_vendor_2, "TSP_ID_CHECK2");
	if (ret < 0) {
		pr_err("%s: Request GPIO failed, gpio %d (%d)\n", BT532_TS_DEVICE,
			tsp_vendor_2, ret);
		return;
	}
	gpio_direction_input(tsp_vendor_1);
	gpio_direction_input(tsp_vendor_2);
	...
	{
		m_firmware_data = m_firmware_data_GFF;
		misc_info->tsp_type = TSP_HW_ID_INDEX_0;
	}
	else
	{
		m_firmware_data = m_firmware_data_G1F;
		misc_info->tsp_type = TSP_HW_ID_INDEX_1;
	}

	if((gpio_get_value(tsp_vendor_1)== 1) && (gpio_get_value(tsp_vendor_2)== 1))
		misc_info->tsp_type = TSP_HW_ID_INDEX_NO_LCD;
...
static int bt532_hw_power(struct bt532_ts_info *info, u8 on)
{
	int ret = 0;
	...
	if (on == 0) {
		...
		if (!is_power_on) {
			is_power_on = 1;
			if (pdata->tsp_supply_type == TSP_REGULATOR_SUPPLY) {
				regulator_set_voltage(touch_regulator, 2800000, 2800000);
...
static bool ts_hw_calibration(struct bt532_ts_info *info)
{
	...
	if (info->tsp_type == TSP_HW_ID_INDEX_NO_LCD)
	{
		pr_info("[TSP] don't do ts_hw_calibration : 0x%x\n", info->tsp_type);
		return true;
	}
...
retry_init:
	...
	if ((checkMode == NULL) &&(ts_check_need_upgrade(info, cap->fw_version,
		...
		if (read_data(client, BT532_DATA_VERSION_REG,
				(u8 *)&cap->reg_data_version, 2) < 0)
			goto fail_init;
		if(read_data(client, BT532_HW_ID,
						(u8 *)&cap->hw_id, 2)< 0)
			goto fail_init;
		if(read_data(client, BT532_VENDOR_ID,
						(u8 *)&cap->vendor_id, 2) < 0)
...
static int bt532_ts_resume(struct device *dev)
{
	...
	if(read_data(client, BT532_FIRMWARE_VERSION, (u8 *)&cap->fw_version, 2));
	if(read_data(client, BT532_MINOR_FW_VERSION, (u8 *)&cap->fw_minor_version, 2))	;
	if(read_data(client, BT532_DATA_VERSION_REG, (u8 *)&cap->reg_data_version, 2));
	if(read_data(client, BT532_HW_ID, (u8 *)&cap->hw_id, 2));
	if(read_data(client, BT532_VENDOR_ID, (u8 *)&cap->vendor_id, 2));
	enable_irq(info->irq);
...
static bool ts_set_touchmode(u16 value)
{
	...
	if(value == TOUCH_DND_MODE) { //SDND mode
		if( misc_info->cap_info.hw_id == TSP_HW_ID_INDEX_0) // Check TSP FW ID
		{   //GFF
			N_cnt = SEC_SDND_N_COUNT_00;
			U_cnt = SEC_SDND_U_COUNT_00;
			freq = SEC_SDND_FREQUENCY_00;
		}
		else
		{   //G1F
			N_cnt = SEC_SDND_N_COUNT_02;
			U_cnt = SEC_SDND_U_COUNT_02;
			freq = SEC_SDND_FREQUENCY_02;
		}
		...
		if( misc_info->cap_info.hw_id == TSP_HW_ID_INDEX_0)  // Check TSP FW ID
		{    //GFF
			N_cnt = SEC_PDND_N_COUNT_00;
			U_cnt = SEC_PDND_U_COUNT_00;
			freq = SEC_PDND_FREQUENCY_00;
		}
		else
		{   //G1F
			N_cnt = SEC_PDND_N_COUNT_02;
			U_cnt = SEC_PDND_U_COUNT_02;
			freq = SEC_PDND_FREQUENCY_02;
         	}
		read_data(misc_info->client, 0x011e,	(u8 *)&reg_val, 2);
		zinitix_bit_clr(reg_val, 1);
		write_reg(misc_info->client, 0x011e,	reg_val);
...
static void get_module_vendor(void *device_data)
{
	...
	case 0:
		module_vendor = 0;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s,%d", "OK", module_vendor); // GFF
		break;
	case 2:
		module_vendor = 1;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s,%d", "OK", module_vendor);  //G1F
		break;
	default: break;
	}
	set_cmd_result(info, finfo->cmd_buff,
...
static void run_reference_PDiff(void *device_data)
{
	...
	*/
	if(check) {
		printk("diff pass\n");
		sprintf(buff, "%d,%d,%d,%d,%s\n",info->hdiff_min_val,info->hdiff_max_val,info->vdiff_min_val,info->vdiff_max_val, "pass");
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		finfo->cmd_state = 2;
	}
	else {
		printk("diff fail\n");
		sprintf(buff,"%d,%d,%d,%d,%s\n",info->vdiff_min_val,info->vdiff_max_val,info->vdiff_min_val,info->vdiff_max_val,"fail");
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		finfo->cmd_state = 3;
	}
			 ...
			 struct bt532_ts_platform_data *pdata)
{
	...
	ret = of_property_read_u16(np, "bt532,x_resolution",
						&pdata->x_resolution);
	if (ret < 0)
		return ret;
	ret = of_property_read_u16(np, "bt532,y_resolution",
						&pdata->y_resolution);
	if (ret < 0)
		return ret;
	ret = of_property_read_u8(np, "bt532,orientation",
						&pdata->orientation);
	if (ret < 0)
		return ret;
	ret = of_property_read_u16(np, "bt532,page_size", &pdata->page_size);
	if (ret < 0)
		return ret;
#if SUPPORTED_TOUCH_KEY_LED
...
#ifdef CONFIG_MACH_DEGAS
	tsp_vendor_1 = of_get_named_gpio(np, "tsp_vendor_1", 0);
	if (tsp_vendor_1 < 0) {
		pr_err("%s: of_get_named_gpio failed: tsp_vendor_1 %d\n", __func__,
			tsp_vendor_1);
		return -EINVAL;
	}
	pdata->tsp_vendor_1 = tsp_vendor_1;

	tsp_vendor_2 = of_get_named_gpio(np, "tsp_vendor_2", 0);
	if (tsp_vendor_2 < 0) {
		pr_err("%s: of_get_named_gpio failed: tsp_vendor_2 %d\n", __func__,
			tsp_vendor_2);
		return -EINVAL;
	}
	pdata->tsp_vendor_2 = tsp_vendor_2;
#endif
...
extern bool sec_bat_is_lpm(void);

static int __init bt532_ts_init(void)
{
	pr_debug("[TSP]: %s lpcharge: %d \n", __func__, sec_bat_is_lpm());
