...
bt_set_evt_filter(bt_private * priv)
{
	...
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q,
		...
		bt_cmd_timeout_func(priv->adapter, BT_CMD_SET_EVT_FILTER);
	}
exit:
	LEAVE();
	return ret;
...
bt_enable_write_scan(bt_private * priv)
{
	...
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q,
		...
		bt_cmd_timeout_func(priv->adapter, BT_CMD_ENABLE_WRITE_SCAN);
	}
exit:
	LEAVE();
	return ret;
...
bt_enable_device_under_testmode(bt_private * priv)
{
	...
	if (!os_wait_interruptible_timeout(priv->adapter->cmd_wait_q,
		...
		bt_cmd_timeout_func(priv->adapter,
				    BT_CMD_ENABLE_DEVICE_TESTMODE);
	}
exit:
	LEAVE();
	return ret;
...
sbi_register_conf_dpc(bt_private * priv)
{
	...
	if (mbt_dev) {
		...
		mbtchar_chown(dev_file, AID_SYSTEM, AID_NET_BT_STACK);  /* AID_BLUETOOTH */
