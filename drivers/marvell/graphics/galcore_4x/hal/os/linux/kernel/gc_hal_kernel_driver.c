...
static int gpu_suspend_prev_notifier_event(struct notifier_block *this,
                        unsigned long event, void *ptr)
{
    gctINT i, ret = NOTIFY_DONE;
    gceSTATUS status;
    gceCHIPPOWERSTATE state;
    gckKERNEL kernel;

    printk("[galcore] enter suspend prepare, event %lu\n", event);
    ...
    switch (event) {
        case PM_SUSPEND_PREPARE:
            for (i = 0, state = -1; i < gcdMAX_GPU_COUNT; i++)
            {
                ...
                printk("[galcore] suspend_prepare: [%d]wait to finish all commands, state %d\n", i, (int)state);
                ...
                if (state == gcvPOWER_ON)
                {
                    ...
                    if (idle == gcvFALSE)
                    {
                        ...
                        {
                            printk("[galcore] [%d] command stall fails\n", i);
                            continue;
                        }
                    }
                }
...
OnError:
    printk("[galcore] exit suspend prepare, rt %d\n", ret);
    return ret;
}

static struct notifier_block suspend_prev_notifier = {
    .notifier_call = gpu_suspend_prev_notifier_event,
};
...
#endif
{
    ...
    if (!ret)
    {
        ...
        register_pm_notifier(&suspend_prev_notifier);
...
#endif
{
    ...
    unregister_pm_notifier(&suspend_prev_notifier);
