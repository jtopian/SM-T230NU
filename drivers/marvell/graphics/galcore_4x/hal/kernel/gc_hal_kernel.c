...
#define DEFAULT_PROFILE_FILE_NAME   "/data/dump/vprofiler.vpd"
#else
#define DEFAULT_PROFILE_FILE_NAME   "/data/dump/vprofiler.xml"
#endif
#else
#if gcdNEW_PROFILER_FILE
    ...
    )
{
...
#endif
    {
        ...
        /* Construct the gckMMU object. */
        gcmkONERROR(
            gckMMU_Construct(kernel, gcdMMU_SIZE, Core, &kernel->mmu));
    ...
    )
{
    ...
    if (Kernel->dbCreated)
    {
             ...
             database = databaseNext)
        {
            ...
            gcmkVERIFY_OK(gcmkOS_SAFE_FREE(Kernel->os, database->htable));
...
** _UseSmallSizePool
**
** Calculate whether reserved memory to reduce memory fragment
**
** The rule is the mon-aligned bytes take the ratio of total size.
**
...
#define gckVIDMEM_AllocateMemory(bContinues, bPmem) \
{\
    if(Kernel->bDisablePmem && bPmem)\
    {\
        status = gcvSTATUS_INVALID_CONFIG;\
    }\
    else\
    {\
        status = gckVIDMEM_ConstructVirtual(Kernel, bContinues, bPmem, Bytes, Node);\
    }\
    if(gcmIS_SUCCESS(status))\
    {\
        (*Node)->Virtual.surfType = Type;\
        *Pool = (bPmem ? gcvPOOL_PMEM : (bContinues ? gcvPOOL_CONTIGUOUS : gcvPOOL_VIRTUAL));\
        break;\
    }\
}

#define gckVIDMEM_AllocateReserve()\
{\
    status = gckVIDMEM_AllocateLinear(Kernel, videoMemory, Bytes, Alignment, Type, Node);\
    if(gcmIS_SUCCESS(status))\
    {\
        (*Node)->VidMem.surfType = Type;\
        *Pool = (*Node)->VidMem.pool = pool;\
        break;\
    }\
}
    ...
    )
{
    ...
    do
    {
        ...
        gcmkERR_BREAK(
            gckKERNEL_GetVideoMemoryPool(Kernel, pool, &videoMemory));

        if((Kernel->hardware->mmuVersion == 0)&&(*Pool == gcvPOOL_CMDBUF))
        {
            if(_UseSmallSizePool(Bytes))
            {
                /* Try to allocate memory from reserve */
                gckVIDMEM_AllocateReserve();
            }

            /* Try to allocate memory from pmem */
            gckVIDMEM_AllocateMemory(gcvTRUE, gcvTRUE);

            /* Try to allocate memory from contiguous memory if above allocate failed. */
            gckVIDMEM_AllocateMemory(gcvTRUE, gcvFALSE);
        }
        else
        {
            if(_UseSmallSizePool(Bytes))
            {
                /* Try to allocate memory from reserve first. */
                gckVIDMEM_AllocateReserve();
            ...
            /* Try to allocate memory from vmalloc if above allocate failed. */
            if((Type != gcvSURF_TILE_STATUS) ||
                ((Type == gcvSURF_TILE_STATUS) &&
                (gcvSTATUS_TRUE == gckHARDWARE_IsFeatureAvailable(Kernel->hardware, gcvFEATURE_MC20))))
            {
                gckVIDMEM_AllocateMemory(gcvFALSE, gcvFALSE);
            }
        }
    }
    while(gcvFALSE);
    ...
    if(gcmIS_SUCCESS(status))
    {
            ...
            && ((*Node)->VidMem.memory->object.type == gcvOBJ_VIDMEM))
        {
            ...
            gckOS_UpdateVidMemUsage(Kernel->os, gcvTRUE, (*Node)->VidMem.bytes,gcvSURF_NUM_TYPES);
            gckOS_UpdateVidMemUsage(Kernel->os, gcvTRUE, (*Node)->VidMem.bytes, (*Node)->VidMem.surfType);
        }
        else
        {
            /* Using aligned size. */
            gckOS_UpdateVidMemUsage(Kernel->os, gcvTRUE, bytesAligned, (*Node)->Virtual.surfType);
        }
    }
    else
    {
        gcmkTRACE(gcvLEVEL_ERROR,
            "%s(%d), _AllocateMemory failed, Type %d, Pool %d, Byte %d, _UseSmallSizePool %d",
            __FUNCTION__, __LINE__,
            Type, *Pool, Bytes, _UseSmallSizePool(Bytes));
    }
    ...
    )
{
    ...
    case gcvHAL_ALLOCATE_LINEAR_VIDEO_MEMORY:
        ...
        if (node->VidMem.memory->object.type == gcvOBJ_VIDMEM)
        {
            ...
            Interface->u.AllocateLinearVideoMemory.physical = 0;
        }
        else
        {
    ...
    case gcvHAL_USER_SIGNAL:
        ...
        case gcvUSER_SIGNAL_DESTROY:
            /*check it in database firstly*/
            gcmkONERROR(gckKERNEL_RemoveProcessDB(
                Kernel,
                processID, gcvDB_SIGNAL,
                gcmINT2PTR(Interface->u.UserSignal.id)));
        ...
        case gcvUSER_SIGNAL_WAIT:
            ...
            gcmkONERROR(
                gckOS_SignalQueryHardware(Kernel->os,
                                          ...
                                          &hardware));
            if ((Interface->u.UserSignal.wait == gcvINFINITE)&&(hardware))
            {
                gctUINT32 timer = 0;
                ...
                for(;;)
                {
                    ...
                    if (status == gcvSTATUS_TIMEOUT)
                    {
                        ...
                        if (timer >= gcdGPU_TIMEOUT)
                        {
                            ...
                            gcmkTRACE_N(
                                ...
                                "%s(%d): kernel dispatch is out of time\n",
                                __FUNCTION__, __LINE__
                                );
                    ...
                    {
                        /* disable the annoying error message when application killed */
                        goto OnError;
                    }
                    else
                    {
    ...
    case gcvHAL_RESET:
        gcmkTRACE_N(gcvLEVEL_ERROR, 0, "gcvHAL_RESET from userspace!!\n");
        /* Reset the hardware. */
    ...
    case gcvHAL_SYNC_POINT:
        {
            ...
            case gcvSYNC_POINT_CREATE:
                ...
                Interface->u.SyncPoint.syncPoint = (gctUINT64)syncPoint;
            ...
            case gcvSYNC_POINT_DESTROY:
                syncPoint = (gctSYNC_POINT)Interface->u.SyncPoint.syncPoint;
    ...
    case gcvHAL_CREATE_NATIVE_FENCE:
        {
            gctINT fenceFD;
            gctSYNC_POINT syncPoint =
                (gctSYNC_POINT)Interface->u.CreateNativeFence.syncPoint;
