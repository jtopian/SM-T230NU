    ...
    )
{
    ...
    Command->waitPhysicalmdl = Command->queues[newIndex].physical;
    Command->waitLogicalBase = Command->queues[newIndex].logical;
    ...
    )
{
    ...
    if((status == gcvSTATUS_TIMEOUT)
        &&  (timer >= Command->kernel->timeOut))
    {
        gckOS_Log(_GFX_LOG_NOTIFY_, "gcdGPU_ADVANCETIMER_STALL gckOS_WaitSignal retried fail ERR");
    }
