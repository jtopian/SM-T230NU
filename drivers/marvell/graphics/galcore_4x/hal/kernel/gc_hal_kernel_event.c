...
OnError:
    ...
    if (eventObj != gcvNULL)
    {
        if (eventObj->pendingLock != gcvNULL)
        {
            gcmkVERIFY_OK(gckOS_DeleteSpinlock(os, eventObj->pendingLock));
        }
    ...
    )
{
    ...
    /* Delete the pending list spinlock. */
    gcmkVERIFY_OK(gckOS_DeleteSpinlock(Event->os, Event->pendingLock));
    ...
    )
{
    gceSTATUS           status      = gcvSTATUS_OK;
    gctBOOL             acquired    = gcvFALSE;
    gctBOOL             maped       = gcvFALSE;
    gcsEVENT_PTR        record      = gcvNULL;
    gctSIGNAL           mapSignal   = gcvNULL;
    gcsEVENT_QUEUE_PTR  queue       = gcvNULL;
    ...
    if(Interface->command == gcvHAL_SIGNAL)
    {
        status = gckOS_MapSignal(Event->os,
                                          Interface->u.Signal.signal,
                                          Interface->u.Signal.process,
                                          &mapSignal);
        maped = gcvTRUE;
    }
...
OnError:
    ...
    if(maped)
    {
        gcmkVERIFY_OK(gckOS_UnmapSignal(Event->os,
                                              Interface->u.Signal.signal));
    }
...
OnError:
    ...
    if (status == gcvSTATUS_GPU_NOT_RESPONDING)
    {
        ...
        gcvLEVEL_ERROR,
        gcmSIZEOF(gctCONST_STRING) + gcmSIZEOF(gctINT),
        "%s(%d): Event submit is out of time\n",
        __FUNCTION__, __LINE__
        );
    ...
    )
{
    ...
    status = gckEVENT_Submit(Event, gcvTRUE, gcvFALSE);
...
OnError:
    while(headRecord != gcvNULL)
    {
        tempRecord = headRecord;
        headRecord = tempRecord->next;
        gcmkVERIFY_OK(gckEVENT_FreeRecord(Event, tempRecord));
    }
    ...
    )
{
    ...
    /* Sync with self or other CPU's soft irq */
    gckOS_Spinlock(Event->os, Event->pendingLock, gcvSPINLOCK_IRQSAVE);
...
/*
    gcmkPRINT("%s is called (%d): pid: %d tid: %d", __func__, __LINE__, tid, pid);
*/
    gcmDEBUG_ONLY(
        if (IDs != 0)
        {
    ...
    {
        gcsEVENT_PTR record;
        /* Disable hard irq on current CPU to prevent preemption,
                   Prevent another CPU to enter this critical section. */
        gckOS_Spinlock(Event->os, Event->pendingLock, gcvSPINLOCK_IRQSAVE);
...
#elif defined(__QNXNTO__)
            atomic_clr((gctUINT32_PTR)&Event->pending, mask);
#else
            Event->pending &= ~mask;
#endif
            gckOS_Spinunlock(Event->os, Event->pendingLock, gcvSPINLOCK_IRQSAVE);
            ...
            case gcvHAL_SYNC_POINT:
                {
                    ...
                    syncPoint = (gctSYNC_POINT)record->info.u.SyncPoint.syncPoint;
...
gceSTATUS
gckEVENT_WaitEmpty(gckEVENT Event)
{
    gctBOOL isEmpty;
