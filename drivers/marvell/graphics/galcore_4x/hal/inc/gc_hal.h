...
#if MRVL_DUMP_GC_INFO
#define gckOS_AcquireMutex(Os, Mutex, Timeout) \
    (gckOS_AcquireMutexEx((Os), (Mutex), (Timeout), (__func__), (__LINE__)))

#define gckOS_ReleaseMutex(Os, Mutex) \
    (gckOS_ReleaseMutexEx((Os), (Mutex), (__func__)))

#define gckOS_AcquireRecMutex(Os, Mutex, Timeout) \
    (gckOS_AcquireRecMutexEx((Os), (Mutex), (Timeout), (__func__), __LINE__))

#define gckOS_ReleaseRecMutex(Os, Mutex) \
    (gckOS_ReleaseRecMutexEx((Os), (Mutex), (__func__), (__LINE__)))
#endif
