...
#define MAJOR_EXT_CLK_ADDR 0xd42828cc
#define TWO_D_EXT_CLK_ADDR 0xd42828f4

#define _GET_EXT_CLK_REGISTER(Vir_addr, Phs_addr, Valure) \
{ \
    Vir_addr = ioremap(Phs_addr, 4); \
    Valure = readl(Vir_addr); \
    iounmap(Vir_addr); \
}
    ...
    )
{
    ...
    gcmkVERIFY_OK(
        gckOS_Broadcast(hardware->os, hardware, gcvBROADCAST_POWER_OFF_TIMEOUT));
}
...
_fetchPulsedata(gckHARDWARE Hardware)
{
    ...
    for(; domain < 2; domain++)
    {
        ...
        /*Count all pulse after one cycle
            ([0,MRVL_PULSE_EATER_COUNT_NUM-1] get busyRatio in ahead records*/
    ...
    IN gceCORE Core)
{
    ...
    {
        _CountPulseRecord(Core, DB, 0, MRVL_PULSE_EATER_COUNT_NUM - 1,
    ...
    else
    {
        _CountPulseRecord(Core, DB, 0, DB->lastIndex - 1,
                          sfPeriod, totalTime, pulseCount);
        _CountPulseRecord(Core, DB, DB->startIndex, MRVL_PULSE_EATER_COUNT_NUM - 1,
                          sfPeriod, totalTime, pulseCount);
    }
...
#if MRVL_ENABLE_VIV_POWER_MANAGEMENT
    ...
    )
{
    ...
    /* Soft reset the GPU. */
    gcmkONERROR(gckOS_WriteRegisterEx(Os, Core, 0x00000, 0x00000900));

    /* Disable auto pulse eater before reading hardware identities. */
    gcmkONERROR(gckOS_WriteRegisterEx(Os,
                Core,
                0x0010C,
	((((gctUINT32)(0x01590880)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) << (0 ? 17:17))) | (((gctUINT32)((gctUINT32)(1)&((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) << (0 ? 17:17)))));
    ...
    hardware->dvfs            = gcvDVFS_ENABLE_INTERNAL;
    hardware->refExtClock     = 1;
    hardware->refExtPower     = 1;
    hardware->outstandingRequest = 0;
    ...
    gcmkONERROR(gckOS_AtomConstruct(Os, &hardware->IsrClkoff));
    gcmkONERROR(gckOS_AtomSet(Os, hardware->IsrClkoff, 1));
...
# if MRVL_CONFIG_SHADER_CLK_CONTROL
        if(has_feat_gc_shader() && Core == gcvCORE_MAJOR)
        {
...
OnError:
    ...
    if (hardware != gcvNULL)
    {
        ...
        if (hardware->IsrClkoff != gcvNULL)
        {
            gcmkVERIFY_OK(gckOS_AtomDestroy(Os, hardware->IsrClkoff));
        }
    ...
    )
{
    ...
    gcmkVERIFY_OK(gckOS_AtomDestroy(Hardware->os, Hardware->IsrClkoff));
    ...
    )
{
	...
	*Offset = (((((gctUINT32)(Address)) >> (0 ? 30:0)) &((gctUINT32) ((((1 ? 30:0) - (0 ? 30:0) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 30:0) - (0 ? 30:0) + 1)))))));
    ...
    {
        /* if new mmu feature support, the msb of address is not mmu flag */
        *Pool = gcvPOOL_SYSTEM;
        *Offset = Address;
    }
...
**          Logical Base address of command buffer.
**
**      gctSIZE_T Bytes
**          Number of bytes for the prefetch unit (until after the first LINK).
**
**      gctPHYS_ADDR Handle
**          the mdl handle of the 'Logical' memory
**
**  OUTPUT:
**
**      Nothing.
    ...
    )
{
    ...
    else
    {
        ...
        if( Logical >= LogicalBase)
        {
            gcmkONERROR(
                gckHARDWARE_ConvertLogicalMdl(Hardware, LogicalBase, &address, Physmdl));
            address= address + ((gctUINT32)Logical - (gctUINT32)LogicalBase);
        }
        else
        {
...
**
**      gctPOINTER LogicalBase
**          Logical Base address of Logical.
**
**      gctUINT32 Offset
    ...
    )
{
    ...
    if (logical != gcvNULL)
    {
        ...
        {
            gcmkONERROR(
                gckHARDWARE_ConvertLogicalMdl(Hardware, LogicalBase, &addressB, Physmdl));
            address= addressB + ((gctUINT32)Logical - (gctUINT32)LogicalBase);
        }
        else
        {
            gcmkONERROR(gcvSTATUS_INVALID_ARGUMENT);
        ...
        Hardware->lastWaitLinkB = addressB;
    ...
    )
{
    ...
    if (Logical != gcvNULL)
    {
        ...
        if( Logical >= Hardware->kernel->command->waitLogicalBase)
        {
            gcmkONERROR(gckHARDWARE_ConvertLogicalMdl(Hardware,
                                                      Hardware->kernel->command->waitLogicalBase,
                                                      &address,
                                                      Hardware->kernel->command->waitPhysicalmdl));
            address= address + ((gctUINT32)Logical - (gctUINT32)(Hardware->kernel->command->waitLogicalBase));
        }
        else
        {
            gcmkONERROR(gcvSTATUS_INVALID_ARGUMENT);
...
**
**      gctPOINTER FetchAddressBase
**          Logical Base address of destination of LINK.
**
**      gctSIZE_T FetchSize
    ...
    )
{
    ...
    if (Logical != gcvNULL)
    {
        ...
        if(FetchAddress >= FetchAddressBase)
        {
            gcmkONERROR(
                gckHARDWARE_ConvertLogicalMdl(Hardware, FetchAddressBase, &address, Physmdl));
            address= address + ((gctUINT32)FetchAddress - (gctUINT32)FetchAddressBase);
    ...
    )
{
    ...
    /* Convert logical address into a physical address. */
    gcmkONERROR(
        gckOS_GetGCPhysByMdl(Hardware->os, Logical, Physmdl, &address));
    ...
    )
{
    ...
    for (retry = 0; retry < 1000; ++retry)
    {
        ...
        for (poll = pollCount; poll > 0; --poll)
        {
            ...
            if ((idle & 0x7FFFFFFF) == 0x7FFFFFFF)
            {
                ...
                /* See if we have reached the last END command */
    ...
    /* out of retry counts and still be NOT idle */
    if (retry >= 1000 && !isIdle)
    {
        gcmkTRACE(gcvLEVEL_WARNING,
                  "%s: GPU is still busy after retry %d times",
                  __FUNCTION__, retry);
    }
    ...
    )
{
    ...
    gctUINT64 time, freq, mutexTime, ntfoffTime, onTime, stallTime, acqTime, stopTime,
              regTime, delayTime, initTime, offTime, startTime, ntfonTime, totalTime;
    gceCHIPPOWERSTATE pre_st, cur_st;
...
#if gcdPOWEROFF_TIMEOUT
    case gcvPOWER_OFF_TIMEOUT_BROADCAST:
        /* Convert to OFF and note we are inside broadcast. */
        State     = gcvPOWER_OFF;
        broadcast = gcvTRUE;
        /* Check time out */
    ...
    */
    ...
    if(Hardware->chipPowerState == gcvPOWER_ON && State == gcvPOWER_OFF)
    {
        if(!((!gckENTRYQUEUE_Qurey(Hardware->kernel->eventObj, &Hardware->kernel->command->queue))))
        {
            gcmkONERROR(gckOS_ReleaseRecMutex(os, Hardware->recMutexPower));
    ...
    if (flag == 0)
    {
        /* Release the recursive power mutex. */
        gcmkONERROR(gckOS_ReleaseRecMutex(os, Hardware->recMutexPower));
    ...
    if((flag & gcvPOWER_FLAG_POWER_OFF) && has_feat_gpufreq())
    {
...
#if MRVL_CONFIG_SHADER_CLK_CONTROL
        if(has_feat_gc_shader() && Hardware->core == gcvCORE_MAJOR)
        {
            gcmkONERROR(gckOS_GPUFreqNotifierCallChain(
                            os,
                            GPUFREQ_GPU_EVENT_POWER_OFF,
    ...
    /* Get time until notified GPUFreq power off. */
    gcmkPROFILE_QUERY(time, ntfoffTime);
    ...
    if ((flag & gcvPOWER_FLAG_STALL) && stall)
    {
        ...
        {
            gctUINT64 rcvy = command->kernel->resetTimeStamp;
            ...
            /* if command stall failed, it will enter kernel recovery to
            *  set power state to off, reset isr and stop cmd, and acquire
            *  powerSemaphore. Skipped the following operations to avoid
            *  acquire powerSemaphore duplicately.
            */
            if(rcvy != command->kernel->resetTimeStamp)
            {
                goto OnError;
            }
        }
    ...
    /* Get time until acquired semaphore. */
    gcmkPROFILE_QUERY(time, acqTime);
    ...
    if (Hardware->clockState && Hardware->powerState)
    {
        ...
        if(gcvSTATUS_TRUE != gckHARDWARE_IsFeatureAvailable(Hardware, gcvFEATURE_DYNAMIC_FREQUENCY_SCALING))
        {
            /*
                ...
                more power on the basis of scaling internal frequency to
                1/64, 2D_DIS/3D_DIS bits would be kept untouched.
             */
    ...
    /* Get time until set register. */
    gcmkPROFILE_QUERY(time, regTime);
    ...
    if (flag & (gcvPOWER_FLAG_POWER_OFF | gcvPOWER_FLAG_CLOCK_OFF))
    {
        if(!gckENTRYQUEUE_Qurey(Hardware->kernel->eventObj, &Hardware->kernel->command ->queue))
        {
            gcmkONERROR(gckOS_AtomSet(os, Hardware->IsrClkoff, 0));

            if(Hardware->core == gcvCORE_MAJOR)
            {
                gctUINT32 outstandingNum;

                if(flag & gcvPOWER_FLAG_CLOCK_OFF)
                {
                    gckOS_ReadRegisterEx(Hardware->os, gcvCORE_MAJOR, 0x00414, &outstandingNum);
                    outstandingNum = outstandingNum & 0x000000FF;
                    if((Hardware->outstandingLimitation != 0)&&
                        (outstandingNum != Hardware->outstandingLimitation))
                    {
                        outstandingNum = Hardware->outstandingLimitation & 0x000000FF;
                    }
                    Hardware->outstandingRequest = outstandingNum;
                }
            }
    ...
    if (flag & gcvPOWER_FLAG_START)
    {
        ...
        /* Set NEW MMU. */
        if (Hardware->mmuVersion != 0 && configMmu && Hardware->kernel->mmu->enabled)
        {
            gcmkONERROR(
                    gckHARDWARE_SetMMUv2(
                        Hardware,
        ...
        if(Hardware->core == gcvCORE_MAJOR)
        {
            ...
            gcmkONERROR(gckOS_ReadRegisterEx(Hardware->os, gcvCORE_MAJOR, 0x00414, &data));
	data = ((((gctUINT32)(data)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 7:0) - (0 ? 7:0) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 7:0) - (0 ? 7:0) + 1))))))) << (0 ? 7:0))) | (((gctUINT32)((gctUINT32)(Hardware->outstandingRequest)&((gctUINT32) ((((1 ? 7:0) - (0 ? 7:0) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 7:0) - (0 ? 7:0) + 1))))))) << (0 ? 7:0)));
	            gcmkONERROR(gckOS_WriteRegisterEx(Hardware->os, gcvCORE_MAJOR, 0x00414, data));
        }
    }
    ...
    if((flag & gcvPOWER_FLAG_INITIALIZE) && has_feat_gpufreq())
    {
        gcmkONERROR(gckOS_GPUFreqNotifierCallChain(
                        os,
    ...
    /* Get time until notified GPUFreq power on. */
    gcmkPROFILE_QUERY(time, ntfonTime);
...
#if gcdENABLE_KERNEL_PROFILING
    pre_st = Hardware->chipPowerState;
    cur_st = State;
...
#if gcdPOWEROFF_TIMEOUT
    if(has_feat_policy_power_off_when_idle())
    {
        ...
        if (State == gcvPOWER_IDLE || State == gcvPOWER_SUSPEND)
        {
            ...
            if((!mrvlPWR)&& (!gckENTRYQUEUE_Qurey(Hardware->kernel->eventObj, &Hardware->kernel->command->queue)))
            {
...
#if gcdENABLE_KERNEL_PROFILING
    gcmkPRINT("PROF(%llu): %d->%d: mutex:%llu ntfoff:%llu on:%llu stall:%llu acq:%llu stop:%llu\n"
              "  reg: %llu delay:%llu init:%llu off:%llu start:%llu ntfon:%llu = total:%llu",
               freq, pre_st, cur_st, mutexTime, ntfoffTime, onTime, stallTime, acqTime, stopTime,
               regTime, delayTime, initTime, offTime, startTime, ntfonTime, totalTime);
    ...
    )
{
	...
	/* Check Idle Register Before Set Isolate bit*/
        if(idle != 0x7FFFFFFF)
        {
            if(!delayed)
            {
                /* If not idle, wait 1 sec*/
                gcmkONERROR(gckOS_Delay(Os, 1000));
                delayed = gcvTRUE;
            ...
            {
                /* If still not idle after waiting ,break it*/
                break;
            }
        }
	...
	if (((((((gctUINT32)(control)) >> (0 ? 16:16)) &((gctUINT32) ((((1 ? 16:16) - (0 ? 16:16) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 16:16) - (0 ? 16:16) + 1))))))) == 0)||  ((((((gctUINT32)(control)) >> (0 ? 17:17)) &((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) == 0))
        ...
        if(Hardware->dvfs == gcvDVFS_DISABLE)
        {
            /* Disable auto pulse eater after soft-reset. */
            gcmkONERROR(gckOS_WriteRegisterEx(Os,
                        Core,
                        0x0010C,
	((((gctUINT32)(0x01590880)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) << (0 ? 17:17))) | (((gctUINT32)((gctUINT32)(1)&((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) << (0 ? 17:17)))));
        ...
        {
            /* We re-enable auto_pulse explictly here, because there may be bug
                that PulseEaterDebug0/1 register won't update even though value of
                PulseEaterReg[17:17] is already 0 after soft-reset.*/
            gcmkONERROR(gckOS_WriteRegisterEx(Os,
                        Core,
                        0x0010C,
	((((gctUINT32)(0x01590880)) & ~(((gctUINT32)(((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) << (0 ? 17:17))) | (((gctUINT32)((gctUINT32)(0)&((gctUINT32) ((((1 ? 17:17) - (0 ? 17:17) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 17:17) - (0 ? 17:17) + 1))))))) << (0 ? 17:17)))));
    ...
    )
{
    ...
    pwrMutexAcquired = gcvTRUE;

    if (Hardware->chipPowerState == gcvPOWER_ON)
    {
        /* Acquire the power management semaphore. */
        status = gckOS_AcquireSemaphore_Timeout(command->os,
                                                command->powerSemaphore,
        ...
        {
            acquired = gcvFALSE;
            gcmkPRINT("[galcore]: acquire power semaphore timeout\n");
        }
    }
...
OnError:
    ...
    if (pwrMutexAcquired)
    {
        /* Release the power management mutex*/
        gcmkVERIFY_OK(
    ...
    )
{
    if(!(has_feat_policy_clock_off_when_idle() ||
         has_feat_policy_power_off_when_idle()))
    {
    ...
    else
    {
        ...
        ** register here; build driver with feature clock_off_when_idle and
        ** power_off_when_idle disabled to make it safe to read mmu register. */
    ...
    )
{
    ...
    gctUINT32 major_clk_ctrl, second_clk_ctrl = 0;
    gctUINT32_PTR major_addr, second_addr;
    ...
    {
        _GET_EXT_CLK_REGISTER(major_addr, MAJOR_EXT_CLK_ADDR, major_clk_ctrl);
        _GET_EXT_CLK_REGISTER(second_addr , TWO_D_EXT_CLK_ADDR, second_clk_ctrl);
    }
    else
    {
        _GET_EXT_CLK_REGISTER(major_addr, MAJOR_EXT_CLK_ADDR, major_clk_ctrl);
    ...
    gcmkPRINT("3D clk register: 0x%08x, 2D clk register: 0x%08x\n", major_clk_ctrl, second_clk_ctrl);

    gcmkPRINT_N(12, "GPU[%d](ChipModel=0x%x ChipRevision=0x%x ChipPowerStates=%d, ClkRefCntEx: %d, IntClk: %d):\n",
                core,
                Hardware->identity.chipModel,
                Hardware->identity.chipRevision,
    ...
    /*FE and LastWaitLink trace*/
    gcmkPRINT("FE0:0x%08X,FE1:0x%08X,lastWaitLink:0x%08X,lastWaitLinkB:0x%08X\n",
                dmaAddress1, dmaAddress2, Hardware->lastWaitLink, Hardware->lastWaitLinkB);

    /*command queueu dump*/
    {
        gctUINT32 address;
        gctUINT32 i,j;
        gctUINT32 *QueueLogic;
        for(i = 0; i < gcdCOMMAND_QUEUES; i++)
        {
            gcmkONERROR(
                gckOS_GetGCPhysByMdl(Hardware->os,
                                     kernel->command->queues[i].logical,
                                     kernel->command->queues[i].physical,
                                     &address));
            gcmkPRINT(">>>CMDqueue i:%d, phys:0x%08x,logc:0x%08x<<<\n",
                      i,
                      address,
                      kernel->command->queues[i].logical);
            QueueLogic = kernel->command->queues[i].logical;
            for(j = 0; j < 256; j++)
            {
                gcmkPRINT("0x%08x: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
                          (address + 0x10*j),
                          QueueLogic[j*4],
                          QueueLogic[j*4 + 1],
                          QueueLogic[j*4 + 2],
    ...
    /*FE location command user buffer dump*/
#if 0
    /*IF only STUCK detect dump or not*/
    if (
        (dmaAddress1 == dmaAddress2)
     && (dmaState1 == dmaState2)
    )
#else
    /*When stop cmd & power off, FE and Other REGs read form Hardware invalidate*/
    if(idle != 0x7FFFFFFF)

#endif
    {
        gctUINT32 physaddr;
        if((dmaAddress1 != 0x0)&&(dmaAddress1 < 0x80000000))
        {
            gctUINT32 *UbufLogic;
            gctUINT32 j;
            gctPHYS_ADDR addr;

            physaddr = ((dmaAddress1 >> PAGE_SHIFT) << PAGE_SHIFT);
            addr = kmap_atomic(phys_to_page(physaddr));
            UbufLogic = (gctUINT32 *)addr;
            gcmkPRINT(">>>dmaAddress1:0x%08x in UCMDbuf phys:0x%08x,logcal:0x%08x<<<\n",
                      dmaAddress1,
                      physaddr,
                      (gctUINT32)addr);
            for(j = 0; j < 256; j++)
        ...
        {
            gcmkPRINT(">>> no search UCMDbuf, more than 0x80000000.dma:0x%08x\n",dmaAddress1);
        }
    }
    /* Restore control. */
    ...
    )
{
    ...
    //gcmkPRINT("[Liang]: shifBits %d, newPeriod: %d, sfPeriod: %d, tmp: %d", shiftBits, newPeriod, Hardware->sfPeriod, defaultPeriod / (gctUINT32)newPeriod);
